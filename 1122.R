...
# 데이터 요약 정보(최소값, 1, 3 분위 수, 중앙값, 평균값, 최대값)
summary(iris)
...
# 의미 있는 값(평균, 중앙값, 최대, 최소값)으로 결측치나 특정 조건을 만족하는 값들을 업데이트(수정) 하는 방법
df3 = df
df3$X[df3$x==0] = 10
...
# left outer join (왼쪽 외부 조인: 왼쪽의 데이터 프레임이 손실되지 않음)
left_join(df1, df2, by = 'ID')

# right outer join ( 오른쪽 외부 조인: 오른쪽의 데이터 프레임이 손실되지 않음)
