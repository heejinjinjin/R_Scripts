...
for(x in 1:iteration) {
  ...
  # 동전이 앞면인 경우에 앞면 횟수 저장하는 변수 count에 1씩 증가
  if(coin == "앞면")
    count = count + 1
...
ggplot(data = df.coin, aes(x = 반복수, y = 누적비율, group = 1)) +
  ...
  labs(title = "동전 던진 횟수에 따른 누적 비율 변화")

# 원주율 구하는 실험 (시뮬레이션)
# 한 변의 길이가 1인 사각형의 1/4의 원이 포함
# 변수 초기화
# 반복 횟수
iteration = 1000
...
for (i in 1:iteration) {
  ...
  # 거리가 1이하면 원 내의 점으로 판단해서 원 내의 점의 개수를 저장하는 N.circle의 값을 누적
  if(dist <= 1){
    N.circle = N.circle + 1
  }
