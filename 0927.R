...
ggplot(df, aes(x = "", y = 초미세먼지농도, fill = 지역)) +
  ...
  scale_fill_manual(values = c("red", "orange", "gold", "darkgreen", "skyblue", "navy", "purple"))

# 히스토그램
# 다수의 data가 있는 경우 특정 값이 나타나는 빈도 수를 그래프로 표현

quakes
# 지진 강도별 밣생 횟수
ggplot(data = quakes, aes(x = mag)) + 
  geom_histogram(col = "black", fill = "pink") +
  labs(title = "지진 강도의 분포", x = "지진 강도", y = "빈도 수")

# 계급의 수 (기본 30개)
# Sturges 공식
# 계습의 수 설정은 histogram 함수에 bins 속성에 계급 수를 설정한다.
...
ggplot(data = quakes, aes(y = mag)) + 
  geom_boxplot(col = "orange", fill = "gold") +
  labs(title = "지진 강도의 분포", x = "피지섬", y = "지진 강도")